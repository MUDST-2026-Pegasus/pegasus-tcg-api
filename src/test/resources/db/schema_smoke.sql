...
-- in the group). Test 1 (ledger sums to wallet.balance) and test 3 (20 concurrent
-- buyers, 1 card) need application code and belong in the Testcontainers suite.
...
VALUES ('ploy@example.com', 'x', 'ploy', 'Ploy');

INSERT INTO seller_profile (user_id, status, verified_at)
SELECT id, 'VERIFIED', now() FROM user_account WHERE username = 'ploy';

INSERT INTO game (code, name, slug) VALUES ('POKEMON', 'Pokemon TCG', 'pokemon');
INSERT INTO catalog_category (game_id, code, name, slug)
SELECT id, 'SINGLES', 'Single Cards', 'singles' FROM game WHERE code = 'POKEMON';
INSERT INTO card_set (game_id, code, name)
SELECT id, 'SV8a', 'Terastal Festival' FROM game WHERE code = 'POKEMON';

INSERT INTO catalog_product (game_id, category_id, card_set_id, name, slug, card_number, attributes)
SELECT g.id, c.id, s.id, 'Pikachu ex', 'pikachu-ex-sv8a-025', '025/187', '{"hp":200}'
FROM game g, catalog_category c, card_set s WHERE g.code='POKEMON' AND c.code='SINGLES' AND s.code='SV8a';

INSERT INTO catalog_variant (catalog_product_id, sku, language_code, finish)
SELECT id, 'PKM-SV8A-025-EN-NORMAL', 'EN', 'NORMAL' FROM catalog_product WHERE slug LIKE 'pikachu%';
INSERT INTO catalog_variant (catalog_product_id, sku, language_code, finish)
SELECT id, 'PKM-SV8A-025-JP-FOIL', 'JP', 'FOIL' FROM catalog_product WHERE slug LIKE 'pikachu%';

\set sp '(SELECT id FROM seller_profile LIMIT 1)'
\set vjp '(SELECT id FROM catalog_variant WHERE sku = \'PKM-SV8A-025-JP-FOIL\')'
\set ven '(SELECT id FROM catalog_variant WHERE sku = \'PKM-SV8A-025-EN-NORMAL\')'
...
VALUES (:sp, :vjp, 'NM', 1290.00, 'ACTIVE', 'lot-a'),
       (:sp, :vjp, 'NM', 1390.00, 'ACTIVE', 'lot-b');
\echo '[PASS] test 4 — two listings, same seller/variant/condition'

\set la '(SELECT id FROM listing WHERE lot_label = \'lot-a\')'
\set lb '(SELECT id FROM listing WHERE lot_label = \'lot-b\')'
...
SELECT lot_label, quantity_total, quantity_available, quantity_reserved FROM listing ORDER BY lot_label;
DO $$
DECLARE a record; b record;
BEGIN
    SELECT * INTO a FROM listing WHERE lot_label = 'lot-a';
    SELECT * INTO b FROM listing WHERE lot_label = 'lot-b';
...
BEGIN
    SELECT quantity_available INTO a FROM listing WHERE lot_label = 'lot-a';
    SELECT quantity_available INTO b FROM listing WHERE lot_label = 'lot-b';
...
UPDATE listing SET price = 1490.00 WHERE lot_label = 'lot-b';
DO $$
DECLARE stale int;
BEGIN
    SELECT count(*) INTO stale FROM listing_unit u JOIN listing l ON l.id = u.listing_id
     WHERE l.lot_label = 'lot-b' AND l.price <> 1490.00;
...
BEGIN
    UPDATE listing_unit SET catalog_variant_id =
        (SELECT id FROM catalog_variant WHERE sku = 'PKM-SV8A-025-EN-NORMAL')
     WHERE id = (SELECT id FROM listing_unit
                  WHERE listing_id = (SELECT id FROM listing WHERE lot_label = 'lot-a')
                  LIMIT 1);
...
BEGIN
    ...
    VALUES ((SELECT id FROM seller_profile LIMIT 1),
            (SELECT id FROM catalog_variant WHERE sku = 'PKM-SV8A-025-JP-FOIL'), 'NM', 'LISTED');
    RAISE EXCEPTION 'should not reach here';
EXCEPTION WHEN check_violation THEN
